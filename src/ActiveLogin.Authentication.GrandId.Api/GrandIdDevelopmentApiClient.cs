...
namespace ActiveLogin.Authentication.GrandId.Api
{
    ...
    public class GrandIdDevelopmentApiClient : IGrandIdApiClient
    {
        ...
        public GrandIdDevelopmentApiClient(List<KeyValuePair<CollectStatus, CollectHintCode>> statusesToReturn)
        ...
        public GrandIdDevelopmentApiClient(string givenName, string surname, List<KeyValuePair<CollectStatus, CollectHintCode>> statusesToReturn)
            : this(givenName, surname, $"{givenName} {surname}", statusesToReturn)
        {
        }

        public GrandIdDevelopmentApiClient(string givenName, string surname, string name)
            : this(givenName, surname, name, DefaultStatusesToReturn)
        {
        }

        public GrandIdDevelopmentApiClient(string givenName, string surname, string name, List<KeyValuePair<CollectStatus, CollectHintCode>> statusesToReturn)
        {
        ...
        private async Task EnsureNoExistingAuth(AuthRequest request)
        {
            if (_auths.Any(x => x.Value.PersonalIdentityNumber == request.PersonalIdentityNumber))
            {
                ...
                throw new GrandIdApiException(ErrorCode.AlreadyInProgress, "A login for this user is already in progress.");
            }
        }
        ...
        public async Task<CollectResponse> CollectAsync(CollectRequest request)
        {
            ...
            {
                throw new GrandIdApiException(ErrorCode.NotFound, "OrderRef not found");
            }
